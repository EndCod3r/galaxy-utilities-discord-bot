...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    if (targetUserRolePosition >= requestUserRolePosition) {
      await interaction.editReply(
        "You can't timeout that member because they have the same/higher role than you!"
      );
      return;
    }
    ...
    if (targetUserRolePosition >= botRolePosition) {
      await interaction.editReply(
        "Unable to timeout the member because they have the same/higher role as the bot!"
      );
      return;
    }
    ...
    try {
      ...
      await targetUser.timeout(msDuration, reason);
      await interaction.editReply(
        `Time out ${targetUser} for ${prettyMs(msDuration, {
          verbose: true,
        })}. \nReason: ${reason}`
      );
    } catch (error) {
  ...
  options: [
    ...
    {
      name: "duration",
      description: "Timeout duration (38m, 1h, 1 day)",
